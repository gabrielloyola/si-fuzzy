...
p.read_fcl_file('kjunior.fcl')  # Parse a file
...
wheelControl = ctrl.ControlSystem(p.rules)
...
while not reachedDestination:
  ...
  try:
    wheelFuzzy.input['Left_Sensor']   = KJuniorProxSensorsVal[0]
    wheelFuzzy.input['Center_Left_Sensor'] = KJuniorProxSensorsVal[1]
    wheelFuzzy.input['Center_Right_Sensor'] = KJuniorProxSensorsVal[2]
    wheelFuzzy.input['Right_Sensor']  = KJuniorProxSensorsVal[3]
  ...
  leftVel  = wheelFuzzy.output['Left_Wheel_Vel']
  rightVel = wheelFuzzy.output['Right_Wheel_Vel']
